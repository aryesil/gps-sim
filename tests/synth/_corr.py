...
Galileo E1 is BOC(1,1): pass ``boc=True`` to :func:`acquire` to multiply the
local E1C-primary replica by the code-locked BOC(1,1) square (sign from
``floor(2 * chip_phase)`` parity, the same construction as the engine mixer)
before the FFT correlation. Without it a plain-primary correlation of a BOC(1,1)
signal still produces peaks but loses several dB and splits ~+/-0.5 chip.
"""
...
def acquire(iq, fs, sysc, prn, *, code_len=None, chip_hz=None,
    ...
    ``boc=True`` multiplies the local replica by the code-locked BOC(1,1)
    square (sign from ``floor(2 * chip_phase)`` parity) -- required to acquire
    a Galileo E1 (BOC(1,1)) capture at its real correlation gain.
    """
    ...
    t = np.arange(npp) / fs
    chip_phase = t * chip_hz
    idx = np.floor(chip_phase).astype(int) % code_len
    local = code[idx].astype(np.float64)
    if boc:
        sub = np.where(np.floor(2.0 * chip_phase).astype(np.int64) & 1, -1.0, 1.0)
        local = local * sub
