...
decoded; the rest returned noise. L5 CNAV is 100 sym/s (IS-GPS-705), so
one symbol spans a single NH10 period."""
import numpy as np
import pytest
...
def _synth(prn, dopp_hz, start_epoch, chip0, syms, dur_s, snr=0.5, seed=5):
    ...
    chips = (i5[idx % _LEN] * _NH10[ep % 10] * syms[ep // 10]).astype(np.float32)
